...
public class AssociateTab
  ...
  public static class HistoryPanel
    ...
    protected void saveModel(ResultItem item) {
      ...
      if (!item.hasModel()) {
        GUIHelper.showErrorMessage(null, "No associator available!");
        return;
      }
        ...
        protected Object doInBackground() throws Exception {
          ...
          for (int i = 0; i < m_Owner.getOutputGenerators().length; i++) {
            try {
              ...
              m_Owner.logError("Failed to generate output with " + m_Owner.getOutputGenerators()[i].toCommandLine(), e, "Associator output generation");
            }
          }
          return null;
        }
	...
	submenu.sort();
      ...
      AbstractHistoryPopupMenuItem.updatePopupMenu(
        ...
        adams.gui.tools.wekainvestigator.tab.associatetab.history.AbstractHistoryPopupMenuItem.class);
  ...
  protected void initialize() {
    ...
    m_CurrentAssociator = null;

    try {
      cmds = OptionUtils.splitOptions(
        props.getProperty("Associate.OutputGenerators", ModelOutput.class.getName()));
    }
    catch (Exception e) {
      ConsolePanel.getSingleton().append(
        Level.SEVERE,
        "Failed to parse output generators:\n" + props.getProperty("Associate.OutputGenerators"), e);
      cmds = new String[]{ModelOutput.class.getName()};
    }
  ...
  protected void initGUI() {
    Associator				cls;
    Class[]				classes;
    AbstractAssociatorEvaluation	eval;
    JPanel				panel;
    Properties				props;
    JPanel				buttonsAll;
    JLabel 				label;
    ...
    try {
      cls = (Associator) OptionUtils.forAnyCommandLine(
        Associator.class,
        InvestigatorPanel.getProperties().getProperty(
          "Associate.Associator", Apriori.class.getName()));
    }
    catch (Exception e) {
      cls = new Apriori();
    }
    m_PanelGOE = new GenericObjectEditorPanel(Associator.class, cls, true);
    m_PanelGOE.setPrefix("Associator ");
    ...
    classes            = AbstractAssociatorEvaluation.getEvaluations();
    for (Class c: classes) {
      try {
        eval = (AbstractAssociatorEvaluation) c.newInstance();
        eval.setOwner(this);
    ...
    m_ComboBoxEvaluations.addActionListener((ActionEvent e) -> {
      if (m_ComboBoxEvaluations.getSelectedIndex() == -1)
        return;
      m_CurrentEvaluation = (AbstractAssociatorEvaluation) m_ComboBoxEvaluations.getSelectedItem();
  ...
  protected void startExecution() {
    m_CurrentAssociator = (Associator) m_PanelGOE.getCurrent();
    startExecution(new InvestigatorTabJob(this, "Starting evaluation '" + m_CurrentEvaluation.getName() + "' using: " + OptionUtils.getCommandLine(m_CurrentAssociator)) {
      @Override
      protected void doRun() {
        ResultItem item;
        try {
          item = m_CurrentEvaluation.init(m_CurrentAssociator);
          m_CurrentEvaluation.addToHistory(m_History, item);
          m_CurrentEvaluation.evaluate(m_CurrentAssociator, item);
          logMessage("Finished evaluation '" + m_CurrentEvaluation.getName() + "' using: " + OptionUtils.getCommandLine(m_CurrentAssociator));
        }
        catch (Exception e) {
          logError("Failed to evaluate associator", e, "Associator evaluation");
          item = null;
        }
        if (item != null)
          generateOutput(item);
  ...
  protected void postStopExecution() {
    ...
    logMessage("Stopped evaluation '" + m_CurrentEvaluation.getName() + "' using: " + OptionUtils.getCommandLine(m_CurrentAssociator));
    updateButtons();
  }
  ...
  public void generateOutput(ResultItem item) {
    for (int i = 0; i < m_OutputGenerators.length; i++) {
      try {
        ...
        logError("Failed to generate output with " + m_OutputGenerators[i].toCommandLine(), e, "Associator output generation");
      }
    }
  }
  ...
  public void updateButtons() {
    ...
    cls = (Associator) m_PanelGOE.getCurrent();
  ...
  protected void doDeserialize(Map<String,Object> data, MessageCollection errors) {
    ...
    if (data.containsKey(KEY_CLUSTERER)) {
      try {
        m_CurrentAssociator = (Associator) OptionUtils.forAnyCommandLine(Associator.class, (String) data.get(KEY_CLUSTERER));
        m_PanelGOE.setCurrent(m_CurrentAssociator);
      }
      catch (Exception e) {
        errors.add("Failed to restore associator: " + data.get(KEY_CLUSTERER), e);
      }
    }
