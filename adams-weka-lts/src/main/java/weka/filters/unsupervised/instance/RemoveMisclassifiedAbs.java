...
implements UnsupervisedFilter, OptionHandler {
	...
	/** for serialization */
	private static final long serialVersionUID = -9062734450276332975L;
	...
	protected double m_numericClassifyThresholdAbs = 1;
		...
		while(count != buildSet.numInstances()) {
			...
			for (int i = 0; i < buildSet.numInstances(); i++) {
				...
				if (buildSet.classAttribute().isNumeric()) {
					if (   (Math.abs(ans - inst.classValue()) < getAbsErr())
							|| (    (ans >= inst.classValue() - inst.classValue()*m_numericClassifyThreshold)
									&& (ans <= inst.classValue() + inst.classValue()*m_numericClassifyThreshold)) ) {
						temp.add(inst);
	...
	private Instances cleanseCross(Instances data) throws Exception {
		...
		Instances temp = new Instances(data, data.numInstances());
		Instances inverseSet = new Instances(data, data.numInstances());
		Instances returnSet = new Instances(data, data.numInstances());
		int removeIndex=-1;
		...
		while (true) {
			...
			iterations++;
			if (m_numOfCleansingIterations > 0 && iterations > m_numOfCleansingIterations) break;
			//returnSet = new Instances(data, data.numInstances());
			...
			for (int fold = 0; fold < m_numOfCrossValidationFolds; fold++) {
				...
				for (int i = 0; i < test.numInstances(); i++) {
					...
					ans  = m_cleansingClassifier.classifyInstance(inst);
					double err=Math.abs(ans - inst.classValue());
			...
			crossSet = returnSet;
			//if (removeIndex >= 0){
			//	returnSet.remove(removeIndex);
			//}
			//crossSet=new Instances(returnSet);
		}
	...
	public Enumeration listOptions() {
		...
		newVector.addElement(new Option(
				"\tThreshold for the max relative error when predicting numeric class.\n"
						+"\t(Value should be >= 0, default = 0.1).",
						"T", 1, "-T <threshold>"));
		newVector.addElement(new Option(
				"\tabsolute error: if abs err is less than this, then OK despite threshold, default = 1).",
				"A", 1, "-A <abs>"));
		newVector.addElement(new Option(
				"\tThe maximum number of cleansing iterations to perform.\n"
   ...
   <!-- options-end -->
	...
	public String [] getOptions() {
		...
		options[current++] = "-A"; options[current++] = "" + getAbsErr();
	...
	public void setAbsErr(double threshold) {
		...
		m_numericClassifyThresholdAbs= threshold;
	...
	public double getAbsErr() {
		...
		return m_numericClassifyThresholdAbs;
	}
	...
	public String thresholdTipText() {
		return "The relative threshold to decide whether classification is ok (percentage, 0-1)";
	}
	...
	public static void main(String [] argv) {
		runFilter(new RemoveMisclassifiedRel(), argv);
	}
}
